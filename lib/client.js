	...
	factory: (require) => {
		...
		*/
		...
		async function installCompanyPackWithCascade(signal) {
			...
			return {
				packEnabled: packResult?.status === "installed" || packResult?.status === "already",
		...
		function CompanyEnterprisePage({ t }) {
			...
			(0, react.useEffect)(() => {
				const controller = new AbortController();
				readInstallations(controller.signal).then((installations) => {
					setCompanyPackEnabled(recommendedPackageInstalled(COMPANY_PACK_RECOMMENDED_ENTRY.packageName, installations));
				}, () => void 0);
				return () => controller.abort();
			}, []);
		...
		const zh = {
			...
			companyPackBody: "可选公司套件通过插件市场安装，不会开机自动装。确认后仅安装套件包本身；工作区与办公 IM 推荐请到同页「精选推荐」。不要把密钥写进包里。",
			pluginCompanyPack: "可选公司套件：示例公司子插件与推荐社区插件。仅在确认后安装。",
			installCompanyPack: "安装公司套件…",
			confirmCompanyPackTitle: "确认安装公司套件",
			confirmCompanyPackBody: "将通过插件市场安装公司套件包。下列子插件与社区推荐为说明用途；继续安装请到「精选推荐」。不会开机自动装。",
			...
			installRestart: "已写入当前 profile。重启 DSH Desktop 后下次启动会加载。",
		...
		const en = {
			...
			companyPackBody: "The optional Company Pack installs through Plugin market and never silent-preinstalls at launch. Confirming installs only the pack package; continue workspace and office picks under Featured on this page. Do not put secrets in the package.",
			...
			confirmCompanyPackBody: "This installs the Company Pack through Plugin market. Child and community rows below are informational; continue installs under Featured. Nothing installs at launch.",
			...
			installRestart: "Wrote the current profile. Restart DSH Desktop so they load on the next startup.",
